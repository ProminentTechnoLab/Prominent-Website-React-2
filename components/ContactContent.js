...
// Local development: 'http://backend.test'
// Production:        'https://api.prominenttechnolabs.com'
const API_URL =
  process.env.NEXT_PUBLIC_API_URL || "https://api.prominenttechnolabs.com";
...
const contactInfo = [
  {
    ...
    title: "Our Office",
    lines: [
      "G-108, Titanium City Center, 100 Feet Anand Nagar Rd, near SACHIN TOWER, Ahmedabad, Gujarat 380015",
    ],
    color: "#FF6600",
  },
  {
    icon: FiPhone,
    title: "Call Us",
    lines: ["+91 93276 03253"],
    href: "tel:+919327603253",
    color: "#0A2463",
  },
  {
    icon: FiMail,
    title: "Email Us",
    lines: ["info@prominenttechnolabs.com"],
    href: "mailto:info@prominenttechnolabs.com",
    color: "#7c3aed",
  },
  {
    icon: FiClock,
    title: "Business Hours",
    lines: ["Mon – Fri: 9:00 AM – 7:00 PM IST"],
    color: "#059669",
  },
];
...
const initialForm = { name: '', email: '', phone: '', subject: '', service: '', message: '' }
...
export default function ContactContent() {
    ...
    const handleChange = e => { setForm(f => ({ ...f, [e.target.name]: e.target.value })); setError(''); setFieldErrors(fe => ({ ...fe, [e.target.name]: '' })) }

    const handleSubmit = async e => {
        e.preventDefault()
        if (!form.name || !form.email || !form.subject || !form.message) { setError('Please fill in all required fields.'); return }
        setLoading(true)
        setError('')
        setFieldErrors({})
        try {
            ...
            if (res.status === 422 && data.errors) {
                ...
                Object.keys(data.errors).forEach(key => {
                    errors[key] = data.errors[key][0] // First error message for each field
                })
            ...
            setError('Unable to connect to the server. Please try again or email us directly at prominenttechnolabs@gmail.com.')
        } finally {
            setLoading(false)
        }
    };
  ...
  return (
    <div>
        ...
        <div className="container">
          ...
          >
            ...
            <p className="page-hero-subtitle">
              Have a project in mind or want to hire a dedicated developer? Drop
              us a message and we'll get back to you within 1 hour.
            </p>
            ...
            {contactInfo.map((c, i) => {
              ...
              return (
                ...
                >
                  ...
                  {c.lines.map((l) =>
                    c.href ? (
                      <a key={l} href={c.href} className="cic-text">
                        {l}
                      </a>
                    ) : (
                      <p key={l} className="cic-text">
                        {l}
                      </p>
                    ),
                  )}
        ...
        <div className="container">
          ...
          >
            <div className="contact-form-left">
              ...
              >
                Fill in the form and we'll get back to you within 1 hour with a
              ...
              >
                {[
                  ...
                  "No contract lock-in for first project",
                  ...
                  >
                    <FiCheckCircle color="#FF6600" size={16} />
                    {f}
                  </div>
                ))}
              </div>
              ...
              >
                ...
                <div
                  style={{ fontSize: "0.82rem", opacity: 0.6, marginTop: 4 }}
                >
            ...
            <div className="contact-form-right">
              {success ? (
                <div className="contact-success">
                  ...
                  <p>
                    Thank you for reaching out. Our team will contact you with a
                    detailed response.
                  </p>
                  <button
                ...
                <form className="contact-form" onSubmit={handleSubmit}>
                  ...
                  <div className="form-row">
                    <div className="form-group">
                      ...
                      <input name="phone" type="tel" className={`form-input ${fieldErrors.phone ? 'input-error' : ''}`} placeholder="+91 93276 03253" value={form.phone} onChange={handleChange} />
                      {fieldErrors.phone && <span className="error-text">{fieldErrors.phone}</span>}
                    </div>
                    <div className="form-group">
                      <label className="form-label">Service Required</label>
                      <select name="service" className={`form-input ${fieldErrors.service ? 'input-error' : ''}`} value={form.service} onChange={handleChange}>
                        <option value="">Select a service...</option>
                        {serviceOptions.map(o => <option key={o}>{o}</option>)}
                      </select>
                      {fieldErrors.service && <span className="error-text">{fieldErrors.service}</span>}
                    </div>
                  </div>
                  <div className="form-group">
                    <label className="form-label">Subject *</label>
                    <input name="subject" type="text" className={`form-input ${fieldErrors.subject ? 'input-error' : ''}`} placeholder="How can we help you?" value={form.subject} onChange={handleChange} required />
      ...
      <style>{`
        ...
        .form-input.input-error { border-color: #dc2626; background-color: #fef2f2; }
        .error-text { font-size: 0.75rem; color: #dc2626; margin-top: 4px; font-weight: 500; }
